...
5. **Captain / vice** for the upcoming GW — ranked on attacking threat × the
   **actual next fixture**, *not* a 5-GW average, so a big name facing a top side
   is correctly demoted below a lesser pick at home to a weak one. The fixture
   ease blends the opponent's defensive strength with the fixture's **FDR** (the
   difficulty the FPL app shows), so it still discriminates even when FPL's team
   strengths are flat/unpublished — FDR is always present. Doubles count twice; a
   player who blanks the upcoming GW is never captained. Shows the shortlist
   (with each fixture's FDR) so you can see why each option ranks where it does.
