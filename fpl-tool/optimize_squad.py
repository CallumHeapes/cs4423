...
# ClubElo uses slightly different club names to FPL; map the mismatches to FPL
# short names (keys are ClubElo names normalised to lowercase, letters only).
ELO_ALIASES = {
    "man united": "MUN", "man utd": "MUN", "manchester united": "MUN",
    "man city": "MCI", "manchester city": "MCI",
    "tottenham": "TOT", "spurs": "TOT",
    "nottingham": "NFO", "nottingham forest": "NFO", "nottm forest": "NFO",
...
def compute_team_outlook(fixtures: list[dict], teams: dict[int, dict],
    ...
    # When FPL's team strengths are unpublished (all 0, as deep pre-season or in
    # this dataset), the strength-derived ease is a flat 1.0 and carries no
    # fixture signal — so fall back to the FDR multiplier, which is always present
    # (and is the difficulty the FPL app itself shows). When strengths ARE
    # published we trust them (a more granular signal than the 1-5 FDR). This
    # keeps fixtures driving the score/captaincy regardless of the odds/ClubElo
    # feeds, which only ever refined the team's own attack multiplier.
    att_pub = avg_def > 0   # opponent-defence ratings available -> real attack ease
    cs_pub = avg_att > 0    # opponent-attack ratings available  -> real CS ease
    ...
    for team_id, rec in acc.items():
        ...
        # that blanks the upcoming GW has no such fixtures -> next_n 0. Summed
        # per fixture so a double GW counts twice; 0 on a blank.
        nx = [i for i, e in enumerate(rec["ev"]) if e == global_next]
        outlook[team_id] = TeamOutlook(
            n_fixtures=n,
            next_n=len(nx),
            next_attack_ease=sum(att_eff[i] for i in nx),
            next_cs_ease=sum(cs_eff[i] for i in nx),
            ...
            attack_ease=_wmean(att_eff, rec["ev"], 1.0),     # near-weighted (score)
            cs_ease=_wmean(cs_eff, rec["ev"], 1.0),
...
def build_players(bootstrap: dict, fixtures: list[dict], horizon: int,
    ...
    # ClubElo team-quality multipliers (preferred over FPL strength when present
    # — a forward-looking signal available before FPL publishes its ratings).
    ...
    for el in bootstrap["elements"]:
        ...
        # Temper attacking output by team quality: ClubElo if we have it, else
        # FPL's own attack-strength rating, else neutral.
        has_elo = el["team"] in elo_mult
        team_att_mult = elo_mult.get(el["team"]) if has_elo \
            else _team_attack_mult(team, avg_att_strength)
        attack_pts *= team_att_mult
        cs_prob = _clean_sheet_prob(el, team, avg_def_strength, cs90, eff_min)
        if has_elo:  # a stronger side keeps more clean sheets, and vice-versa
            cs_prob = _clamp(cs_prob * team_att_mult, *CS_PROB_CLAMP)
